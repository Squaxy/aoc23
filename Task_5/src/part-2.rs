...
fn main() {
    ...
    let mut inital_seeds = Vec::<u64>::new();
    ...
    println!("created initial seeds!");

    let mut seeds = Vec::<u64>::new();
    let mut i = 0;
    while i < inital_seeds.len() {
        let seed_start = *inital_seeds.get(i).unwrap();
        let seed_range = *inital_seeds.get(i+1).unwrap();
    ...
    println!("created part-2 seeds! There are {} many seeds!", seeds.len());
    ...
    println!("created gardeners_maps!");

    println!("Now going to start the seed propagation!");
    let seeds_to_location_map: HashMap<u64, u64> = seeds.iter()
    .map(|&seed| (seed, propagate_seed(seed, &gardeners_maps)))
    .collect();

    println!("Done with seed propagation!");
